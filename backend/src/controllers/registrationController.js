...
export const downloadCertificate = asyncHandler(async (req, res) => {
  ...
  if (isWorkshop) {
    ...
    const isWinner = String(sub.winnerRegistration || "") === String(reg._id);
    const isRunner = String(sub.runnerRegistration || "") === String(reg._id);
    if (!isWinner && !isRunner) {
      res.status(400);
      throw new Error("Competitive certificate is available only for winner/runner");
    }
    certificateType = isWinner ? "winner" : "runner";
    position = isWinner ? "1st" : "2nd";
  }
